...
p22<-ggplot(dat22, aes(X22)) +
  labs(title = "Profile Nr.22", x = "Depths, m", y = "Density") +
  theme(
  ...
  scale_x_continuous(breaks = pretty(dat22$X22, n = 4), minor_breaks = seq(min(dat22$X22), max(dat22$X22), by = 500)) +
  ...
  geom_histogram(binwidth = 200,aes(fill = ..density..,x = dat22$X22,y = ..density..),color = "blue",size = .1) +
  ...
  stat_density(geom = "line", size = .3, aes(color = "density")) +
	     ...
	     geom_vline(aes(color = "norm_dist", xintercept = dnorm(X22)), lty = 2, size = .3)
p22
