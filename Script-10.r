...
p10<-ggplot(dat10, aes(X10)) +
  labs(title = "Profile Nr.10", x = "Depths, m", y = "Density") +
  theme(
  ...
  scale_x_continuous(breaks = pretty(dat10$X10, n = 4), minor_breaks = seq(min(dat10$X10), max(dat10$X10), by = 500)) +
  ...
  geom_histogram(binwidth = 200,aes(fill = ..density..,x = dat10$X10,y = ..density..),color = "blue",size = .1) +
  ...
  stat_density(geom = "line", size = .3, aes(color = "density")) +
	     ...
	     geom_vline(aes(color = "norm_dist", xintercept = dnorm(X10)), lty = 2, size = .3)
p10
