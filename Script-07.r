...
p07<-ggplot(dat07, aes(X07)) +
  labs(title = "Profile Nr.07", x = "Depths, m", y = "Density") +
  theme(
  ...
  scale_x_continuous(breaks = pretty(dat07$X07, n = 4), minor_breaks = seq(min(dat07$X07), max(dat07$X07), by = 500)) +
  ...
  stat_density(geom = "line", size = .3, aes(color = "density")) +
	     ...
	     geom_vline(aes(color = "norm_dist", xintercept = dnorm(X07)), lty = 2, size = .3)
p07
