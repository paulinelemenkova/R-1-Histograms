...
p20<-ggplot(dat20, aes(X20)) +
  labs(title = "Profile Nr.20", x = "Depths, m", y = "Density") +
  theme(
  ...
  scale_x_continuous(breaks = pretty(dat20$X20, n = 4), minor_breaks = seq(min(dat20$X20), max(dat20$X20), by = 500)) +
  ...
  geom_histogram(binwidth = 200,aes(fill = ..density..,x = dat20$X20,y = ..density..),color = "blue",size = .1) +
  ...
  stat_density(geom = "line", size = .3, aes(color = "density")) +
	     ...
	     geom_vline(aes(color = "norm_dist", xintercept = dnorm(X20)), lty = 2, size = .3)
p20
