...
p08<-ggplot(dat08, aes(X08)) +
  labs(title = "Profile Nr.08", x = "Depths, m", y = "Density") +
  theme(
  ...
  scale_x_continuous(breaks = pretty(dat08$X08, n = 4), minor_breaks = seq(min(dat08$X08), max(dat08$X08), by = 500)) +
  ...
  stat_density(geom = "line", size = .3, aes(color = "density")) +
	     ...
	     geom_vline(aes(color = "norm_dist", xintercept = dnorm(X08)), lty = 2, size = .3)
p08
