...
p03<-ggplot(dat03, aes(X03)) +
  labs(title = "Profile Nr.03", x = "Depths, m", y = "Density") +
  theme(
  ...
  scale_x_continuous(breaks = pretty(dat03$X03, n = 4), minor_breaks = seq(min(dat03$X03), max(dat03$X03), by = 500)) +
  ...
  stat_density(geom = "line", size = .3, aes(color = "density")) +
	     ...
	     geom_vline(aes(color = "norm_dist", xintercept = dnorm(X03)), lty = 2, size = .3)
p03
