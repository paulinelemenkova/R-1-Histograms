...
p24<-ggplot(dat24, aes(X24)) +
  labs(title = "Profile Nr.24", x = "Depths, m", y = "Density") +
  theme(
  ...
  scale_x_continuous(breaks = pretty(dat24$X24, n = 4), minor_breaks = seq(min(dat24$X24), max(dat24$X24), by = 500)) +
  ...
  geom_histogram(binwidth = 200,aes(fill = ..density..,x = dat24$X24,y = ..density..),color = "blue",size = .1) +
  ...
  stat_density(geom = "line", size = .3, aes(color = "density")) +
	     ...
	     geom_vline(aes(color = "norm_dist", xintercept = dnorm(X24)), lty = 2, size = .3)
p24
