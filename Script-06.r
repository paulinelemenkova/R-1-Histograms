...
p06<-ggplot(dat06, aes(X06)) +
  labs(title = "Profile Nr.06", x = "Depths, m", y = "Density") +
  theme(
  ...
  scale_x_continuous(breaks = pretty(dat06$X06, n = 4), minor_breaks = seq(min(dat06$X06), max(dat06$X06), by = 500)) +
  ...
  stat_density(geom = "line", size = .3, aes(color = "density")) +
	     ...
	     geom_vline(aes(color = "norm_dist", xintercept = dnorm(X06)), lty = 2, size = .3)
p06
