...
p15<-ggplot(dat15, aes(X15)) +
  labs(title = "Profile Nr.15", x = "Depths, m", y = "Density") +
  theme(
  ...
  scale_x_continuous(breaks = pretty(dat15$X15, n = 4), minor_breaks = seq(min(dat15$X15), max(dat15$X15), by = 500)) +
  ...
  geom_histogram(binwidth = 200,aes(fill = ..density..,x = dat15$X15,y = ..density..),color = "blue",size = .1) +
  ...
  stat_density(geom = "line", size = .3, aes(color = "density")) +
	     ...
	     geom_vline(aes(color = "norm_dist", xintercept = dnorm(X15)), lty = 2, size = .3)
p15
