...
p19<-ggplot(dat19, aes(X19)) +
  labs(title = "Profile Nr.19", x = "Depths, m", y = "Density") +
  theme(
  ...
  scale_x_continuous(breaks = pretty(dat19$X19, n = 4), minor_breaks = seq(min(dat19$X19), max(dat19$X19), by = 500)) +
  ...
  stat_function(fun = dnorm, args = list(mean = mean(dat19$X19), sd = sd(dat19$X19)), lwd = 0.2, color = 'black') +
  stat_density(geom = "line", size = .3, aes(color = "density")) +
	     ...
	     geom_vline(aes(color = "norm_dist", xintercept = dnorm(X19)), lty = 2, size = .3)
p19
