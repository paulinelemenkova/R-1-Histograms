...
p18<-ggplot(dat18, aes(X18)) +
  labs(title = "Profile Nr.18", x = "Depths, m", y = "Density") +
  theme(
  ...
  scale_x_continuous(breaks = pretty(dat18$X18, n = 4), minor_breaks = seq(min(dat18$X18), max(dat18$X18), by = 500)) +
  ...
  geom_histogram(binwidth = 200,aes(fill = ..density..,x = dat18$X18,y = ..density..),color = "blue",size = .1) +
  ...
  stat_density(geom = "line", size = .3, aes(color = "density")) +
	     ...
	     geom_vline(aes(color = "median", xintercept = median(X18)), lty = 2, size = .3)  +
	     geom_vline(aes(color = "norm_dist", xintercept = dnorm(X18)), lty = 2, size = .3)
p18
