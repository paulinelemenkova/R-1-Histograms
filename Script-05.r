...
p05<-ggplot(dat05, aes(X05)) +
  labs(title = "Profile Nr.05", x = "Depths, m", y = "Density") +
  theme(
  ...
  scale_x_continuous(breaks = pretty(dat05$X05, n = 4), minor_breaks = seq(min(dat05$X05), max(dat05$X05), by = 500)) +
  ...
  stat_density(geom = "line", size = .3, aes(color = "density")) +
	     ...
	     geom_vline(aes(color = "norm_dist", xintercept = dnorm(X05)), lty = 2, size = .3)
p05
