...
p17<-ggplot(dat17, aes(X17)) +
  labs(title = "Profile Nr.17", x = "Depths, m", y = "Density") +
  theme(
  ...
  scale_x_continuous(breaks = pretty(dat17$X17, n = 4), minor_breaks = seq(min(dat17$X17), max(dat17$X17), by = 500)) +
  ...
  geom_histogram(binwidth = 200,aes(fill = ..density..,x = dat17$X17,y = ..density..),color = "blue",size = .1) +
  ...
  stat_density(geom = "line", size = .3, aes(color = "density")) +
	     ...
	     geom_vline(aes(color = "norm_dist", xintercept = dnorm(X17)), lty = 2, size = .3)
p17
